...
`graph.jsonld` is mirrored on Hugging Face Datasets (primary ingest source for LLM training pipelines and knowledge-graph crawlers; auto-converted to Parquet, loadable directly from `pandas` / `Polars`). Sync via the `hf-sync` skill: run `/hf-sync <Owner/dataset>` (or `bash ~/.Codex/skills/hf-sync/sync.sh <Owner/dataset>`) from the project root, typically as the last step of the `release-doi` flow after `gh release create`. Local-only — uses the user's `hf login` token, no CI auth required.
