...
- `vocab.html` — dereference target for the `shimo:` vocabulary namespace (`…/vocab#Term`). Update only when a graph introduces a new class/property or a new hub-level `Concept` node; routine releases do not touch it. Its `<head>` JSON-LD block is a mirror of `vocab.jsonld` — **do not hand-edit the block**; edit `vocab.jsonld` and re-run `scripts/sync_vocab_jsonld_mirror.py`. Keep the human-readable table definitions and `vocab.jsonld` `rdfs:comment` values saying the same thing when either changes
- `vocab.jsonld` — canonical machine-readable layer of the vocabulary (RDFS + `schema:domainIncludes`/`rangeIncludes`; deliberately no OWL axioms). Served adjacent to the page (`…/vocab.jsonld`, referenced via `<link rel="alternate">`) because GitHub Pages cannot content-negotiate. Term `@id`s must match the `vocab.html` row anchors 1:1; definitions are induced from actual graph usage, not invented
